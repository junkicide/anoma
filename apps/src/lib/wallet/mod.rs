...
pub struct Wallet {
    store_dir: PathBuf,
    store: Store,
    decrypted_key_cache: HashMap<Alias, Rc<ed25519c::Keypair>>,
}
...
impl Wallet {
    ...
    ) -> Result<Rc<ed25519c::Keypair>, FindKeyError> {
        // Try cache first
    ...
    fn decrypt_stored_key(
        decrypted_key_cache: &mut HashMap<String, Rc<ed25519c::Keypair>>,
