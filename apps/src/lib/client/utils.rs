...
pub fn init_network(
    ...
    config.validator.iter_mut().for_each(|(name, config)| {
        ...
        let tm_node_keypair_json = ed25519c::SecretKey::try_from_sk(&node_seckey).map(|sk| {
            ...
            let node_keypair =
                [sk.try_to_vec().unwrap(),
                 sk.ref_to().try_to_vec().unwrap()].concat();
            json!({
                "priv_key": {
        ...
        config.staking_reward_public_key =
            Some(genesis_config::HexString(reward_keypair.ref_to().to_string()));
...
fn init_established_account(
    ...
    if config.public_key.is_none() {
        ...
        let public_key = genesis_config::HexString(keypair.ref_to().to_string());
        config.public_key = Some(public_key);
    }
    if config.vp.is_none() {
...
fn init_genesis_validator_aux(
    ...
    let genesis_validator = genesis::Validator {
        pos_data: anoma::ledger::pos::GenesisValidator {
            ...
            consensus_key: consensus_key.ref_to().clone(),
            staking_reward_key: rewards_key.ref_to().clone(),
        },
        account_key: validator_key.ref_to().clone(),
